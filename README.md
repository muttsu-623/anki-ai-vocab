...
A Dockerized command-line tool that automatically adds English vocabulary and expressions to your Anki deck with AI-generated definitions, pronunciation, and examples.

## Features

- Fetches expression information using OpenAI's GPT model:
  - Japanese meaning
  - English definition with parts of speech [noun], [verb], [adjective], etc.
  - IPA pronunciation
  - Related idioms/phrases
  - Example sentences
  - Similar expressions with differences
- **Automatic audio generation** using Amazon Polly:
  - Expression pronunciation with clear, slower speech
...
- **Card management**:
  ...
  - Delete existing cards by expression search
  - Automatic deck creation if needed
- **Interactive mode** for continuous expression processing
...
Add an expression to your Anki deck (audio is generated automatically):
```bash
...
Enter interactive mode for continuous expression processing:
```bash
...
docker compose run --rm anki-vocab "participate in" --japanese-meaning "参加する"
```

**Delete an Expression:**
```bash
docker compose run --rm anki-vocab "serendipity" --delete
docker compose run --rm anki-vocab "participate in" --delete
```
...
- **Front**: Expression with audio playback, IPA pronunciation
- **Back**:
...
- **"Duplicate card"**:
  - The expression already exists in your deck
